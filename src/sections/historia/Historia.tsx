...
const Historia: React.FC<HistoriaProps> = ({ className }) => {
  const { contentJson } = useContext(IdiomaContext);

  return (
    <section
      style={{
        backgroundImage: `linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url(${contentJson.historia.imagen})`,
      }}
      className={`
        bg-cover bg-center bg-no-repeat h-[600px] flex flex-col flex-wrap
      ...
      <div>
        <Boton where="/historia">
          <span className="font-medium text-base sm:text-sm md:text-base lg:text-lg">
            {contentJson.historia.botonVerMas}
          </span>
        </Boton>
      </div>
    </section>
